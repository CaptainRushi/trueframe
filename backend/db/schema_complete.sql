-- ====================================================================
-- TRUEFRAME PLATFORM - COMPLETE CONSOLIDATED DATABASE SCHEMA
-- ====================================================================
-- This file merges schema.sql, migration_identity_verification.sql, 
-- and migration_secondary_review.sql into a single executable script.
-- Recommended for direct paste into Supabase SQL Editor.
-- ====================================================================
...
CREATE TABLE IF NOT EXISTS profiles (
    id UUID PRIMARY KEY, -- Corresponds to Supabase Auth user ID
    username VARCHAR(50) UNIQUE NOT NULL, -- Replaced with deleted_user_<uuid> upon account deletion
...
-- Secondary Reviews Table (Post-Flag AI Review Pipeline)
CREATE TABLE IF NOT EXISTS secondary_reviews (
    ...
    -- Model 2 result (populated when Model 1 is borderline 0.60-0.80)
    -- Model 2 = main.py HuggingFace+signal ensemble for a second independent opinion
    model2_score      FLOAT,
    model2_signals    TEXT[],
    model2_decision   VARCHAR(20)
...
-- Community Flags (With Flag Weight and Triggered Review fields integrated)
CREATE TABLE IF NOT EXISTS community_flags (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    post_id UUID NOT NULL REFERENCES posts(id) ON DELETE CASCADE,
    flagger_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
    flag_type VARCHAR(30) NOT NULL CHECK (flag_type IN ('MISINFORMATION', 'MANIPULATED', 'OUT_OF_CONTEXT', 'SPAM', 'AI_GENERATED', 'CARTOON', 'OTHER')),
    reason TEXT,
    source_url TEXT,
    ...
    reviewed_at TIMESTAMP WITH TIME ZONE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    flag_weight FLOAT DEFAULT 1.0,
    triggered_review BOOLEAN DEFAULT FALSE,
    UNIQUE(post_id, flagger_id)
);

-- Notifications (With expanded check constraints for Secondary Review and Trust Penalty alerts)
...
-- Content Proofs (Verification proofs for decentralized content checking)
...
-- Trust Stats Trigger & Functions (Calculated based on verification logs)
