...
export function AppLayout() {
  ...
  const isProfileOrDashboard = location.pathname.includes('/profile') || location.pathname.includes('/dashboard');
  ...
  return (
    <div className="min-h-screen bg-background relative w-full overflow-x-hidden flex justify-center">
      ...
      <div className="w-full max-w-[1200px] mx-auto flex flex-row relative justify-center">
        ...
        {/* Center Main Content */}
        <main 
          className={`flex-1 min-w-0 min-h-screen w-full pb-20 md:pb-0 relative border-x border-border/50 bg-background transition-all ${
            isProfileOrDashboard ? 'max-w-[900px]' : 'max-w-[600px]'
          }`}
        >
          <Outlet />
        </main>
