...
export function AppLayout() {
  return (
    <div className="min-h-screen bg-background relative w-full overflow-x-hidden flex justify-center">
      ...
      <div className="w-full max-w-7xl mx-auto flex flex-row relative justify-center lg:gap-4 xl:gap-8 px-0 md:px-4 lg:px-8">
        
        {/* Left Sidebar - sticky so it doesn't scroll, stays in flow */}
        <div className="hidden md:flex sticky top-0 h-screen w-[275px] min-w-[275px] shrink-0 z-40 xl:border-r border-border/50">
          <SideNav />
        </div>

        {/* Center Main Content */}
        <main className="flex-1 min-w-0 min-h-screen max-w-[600px] w-full pb-20 md:pb-0 relative xl:border-r border-border/50">
          <Outlet />
        </main>

        {/* Right Sidebar Placeholder (Balance for desktop/XL screens) */}
        <div className="hidden xl:block sticky top-0 h-screen w-[320px] min-w-[320px] shrink-0 p-6 pt-10">
